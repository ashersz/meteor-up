...
function copyConfigure(tasklist, env, appName, appRemote, appUser, appLog, meteor_settings) {
  ...
  var env = _.clone(env);
  var appPort = env.PORT || 3000;
  var appRootUrl = env.ROOT_URL || "127.0.0.1";
  if (appPort!==3000){
      appRootUrl += ":"+appPort;
  }
  var clusterBalancerUrl = env.CLUSTER_BALANCER_URL?"Environment=CLUSTER_BALANCER_URL="+env.CLUSTER_BALANCER_URL:"";
  tasklist.copy('Configuring systemd daemon...', {
    ...
    vars: {
      ...
      clusterBalancerUrl:clusterBalancerUrl
    }
  });

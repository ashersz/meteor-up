...
function buildApp(appPath, build_env, buildLocaltion, buildOptions, callback) {
  buildMeteorApp(appPath, build_env, buildLocaltion, buildOptions, function(code) {
    if(code == 0) {
...
function buildMeteorApp(appPath, build_env, buildLocaltion, buildOptions, callback) {
  ...
  for (var prop in build_env){
      console.log("build_env." + prop + " = " + JSON.stringify(build_env[prop]));
      process.env[prop]=JSON.stringify(build_env[prop]);
  }

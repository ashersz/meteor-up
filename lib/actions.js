var nodemiral_forcetty = require('nodemiral-forcetty');
...
function Actions(config, cwd, options) {
  ...
  if(fs.existsSync(setttingsJsonPath)) {
    ...
    this.config.env['METEOR_SETTINGS'] = "";
  }
}
...
Actions.prototype._createSessionsMap = function(config) {
  ...
  config.servers.forEach(function(server) {
    ...
    var session = server.os!=="centos"?nodemiral.session(host, auth, nodemiralOptions):nodemiral_forcetty.session(host, auth, options);
...
//  self._showKadiraLink();
  ...
  var build_env = this.config.build_env;
  console.log("build_env="+JSON.stringify(build_env));
  console.log('Building Started: ' + this.config.app);
  buildApp(appPath, build_env, meteorBinary, buildLocation, function(err) {
    if(err) {
      process.exit(1);
    } else {
      var sessionsData = [];
  ...
  for(var os in self.sessionsMap) {
    ...
    sessionsInfo.sessions.forEach(function(session) {
      ...
      var taskList = sessionsInfo.taskListsBuilder.reconfig(
        env, self.config.appName,self.config.appRemote,self.config.appUser, self.config.appLog, self.config.deployCheckWaitTime, self.config.env['METEOR_SETTINGS']);
...
Actions.prototype.restart = function() {
  ...
  for(var os in self.sessionsMap) {
    ...
    sessionsInfo.sessions.forEach(function(session) {
      if(os == 'centos') {
        console.log("Restarting "+self.config.appName);
        var command = 'sudo systemctl restart ' + self.config.appName + '.service';
        var options = {};
        session.execute(command, options);
      } else {
          var env = _.extend({}, self.config.env, session._serverConfig.env);
          self._executePararell("restart", [env,self.config.appName,self.config.appRemote]);
      }
    });
  }
};
...
Actions.prototype.stop = function() {
  this._executePararell("stop", [this.config.appName,this.config.appRemote]);
};
...
Actions.prototype.start = function() {
  ...
  for(var os in self.sessionsMap) {
    ...
    sessionsInfo.sessions.forEach(function(session) {
      if(os == 'centos') {
        console.log("Starting "+self.config.appName);
        var command = 'sudo systemctl start ' + self.config.appName + '.service';
        var options = {};
        session.execute(command, options);
      } else {
          var env = _.extend({}, self.config.env, session._serverConfig.env);
          self._executePararell("start", [env,self.config.appName,self.config.appRemote]);
      }
    });
  }
};
    ...
    sessionsInfo.sessions.forEach(function(session) {
      ...
      if(os == 'linux') {
        ...
        var command = 'sudo tail ' + tailOptions +
          ...
          var command = 'sudo tail ' + tailOptions + self.config.appLog ;
      }
      session.execute(command, options);
    });
  }
